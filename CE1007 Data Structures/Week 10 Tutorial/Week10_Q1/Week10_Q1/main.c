...
int removeNode(ListNode **ptrHead,int index)
{
    ListNode *pre, *cur; // Creating new structures for pre and cur
    ...
    if ((pre = findNode(*ptrHead, index-1)) != NULL) { // means the node at the specified target index does not exist
        if (pre->next == NULL) { // means pre is the last node of the list.
            return 0;
        }
        else {
            cur = pre->next;
